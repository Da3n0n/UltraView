...
if (existsSync(archive) && existsSync(archiveMarker)) {
    ...
    if (prepared.version === pin.version && prepared.commit === pin.commit && prepared.platform === platformKey) {
...
writeFileSync(join(stagingRoot, 'runtime.json'), `${JSON.stringify({ version: pin.version, commit: pin.commit, platform: platformKey }, null, 4)}\n`);
...
writeFileSync(archiveMarker, `${JSON.stringify({ version: pin.version, commit: pin.commit, platform: platformKey }, null, 4)}\n`);
rmSync(stagingRoot, { recursive: true, force: true, maxRetries: 4, retryDelay: 250 });
