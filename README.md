...
No drivers. No heavy clients. No hassle. Ultraview brings a full suite of viewers, editors, and developer tools right inside VS Code — and it always looks like it belongs.
...
|---|---|
| 🗃️ **Database Viewer** | Browse SQLite, DuckDB, Access, SQL dumps, index files — no external client |
| 📝 **Markdown Editor** | Full WYSIWYG editor with Rich / Raw / Split view, toolbar, and auto-save |
| 🕸️ **Interactive Code Graph** | Obsidian-style node graph of your entire codebase with physics simulation |
| 🔐 **Git Account Manager** | Manage GitHub / GitLab / Azure accounts, SSH keys, and tokens per-project |
| 🔄 **Cross-IDE Project Sync** | One shared project list and account manager across VS Code, Cursor, Windsurf, etc. |
| 🎨 **Dynamic Theming** | Every panel adapts to your active VS Code theme — light, dark, or any custom theme |
| 🌐 **Quick URL Opener** | Open any URL in VS Code's Simple Browser from the Command Palette |
| 💬 **Custom Comment Fonts** | Style comments with any locally installed font |
...
```
C:\Users\You\.ultraview\
  sync.json     ← shared project list, accounts, profiles (no tokens)
```
...
2. Run **`Ultraview: Set Cross-IDE Sync Folder`**
3. Browse to your preferred folder or choose **Use default (~/.ultraview)**
...
1. Install Ultraview in **IDE A** (e.g. VS Code). Add your accounts and projects. The sync file is created automatically at `~/.ultraview/sync.json`.
2. Install Ultraview in **IDE B** (e.g. Cursor). On first activation it migrates any local data and reads from the same `~/.ultraview/sync.json`.
3. That's it — both IDEs now share the same project list and accounts.
...
| Project paths, profiles | `~/.ultraview/sync.json` (plain text, safe) |
| Auth tokens (PAT / OAuth) | OS keychain via VS Code `context.secrets` — **never** in the JSON |
| SSH private keys | OS keychain via VS Code `context.secrets` — **never** in the JSON |
...
2. **Open a file** — double-click a `.db`, `.sqlite`, `.md`, or other supported file in your explorer
3. **Open the Code Graph** — Command Palette → `Ultraview: Open Code Graph`
4. **Open the Git panel** — click the Git icon in the activity bar, or run `Ultraview: Open Git Projects Manager`
5. **Add your first account** — in the Git panel, click **+ Add Account** and choose browser OAuth or PAT
6. **Cross-IDE sync** — install Ultraview in your other IDEs (Cursor, Windsurf, etc.). Your projects and accounts appear automatically from `~/.ultraview/sync.json`
7. **Open any URL** — Command Palette → `Ultraview: Open URL`
...
1. On `activate()`, it loads `~/.ultraview/sync.json` into memory
2. A `fs.watch` listener hot-reloads data when another process writes the file
3. All writes go through a 100 ms debounce + atomic rename (`sync.json.tmp` → `sync.json`) to avoid corruption
4. Tokens and SSH private keys are stored only in `context.secrets` and referenced by ID
5. On first run, existing `globalState` data is automatically migrated into the shared file
...
**Enjoy Ultraview** — feedback and issues welcome on [GitHub](https://github.com/Da3n0n/UltraView).
